...
def agent_select_action(
    ...
    active  = [z for z in obs.get("zones", []) if z.get("severity") not in ("none", "low")]
    zone_id = active[0]["zone_id"] if active else (
        obs["zones"][0]["zone_id"] if obs.get("zones") else "Z1"
    )
    return {
...
def run_task(task_id: str, client: OpenAI, model_name: str) -> Dict:
    ...
    while not done and step_num < MAX_STEPS:
        try:
            ...
            action_str = (
                f"{action['action_type']}(zone={action['zone_id']},units={action.get('units',1)})"
            )
            ...
            step_num  += 1
            rewards.append(0.0)
            log_step(step=step_num, action="null", reward=0.0, done=False, error=last_error)
