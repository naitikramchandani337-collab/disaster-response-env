...
TASKS["task_1_earthquake"] = TaskDefinition(
    ...
    zones = [
        ZoneState(
            zone_id="Z1", name="Downtown Core",
            disaster_type=DisasterType.EARTHQUAKE,
            severity=ZoneSeverity.CRITICAL,
            population=5000, trapped_casualties=80, injured=120,
            rescued=0, fatalities=0, accessibility=0.6
        ),
        ZoneState(
            zone_id="Z2", name="Residential North",
            disaster_type=DisasterType.EARTHQUAKE,
            severity=ZoneSeverity.HIGH,
            population=8000, trapped_casualties=45, injured=200,
            rescued=0, fatalities=0, accessibility=0.8
        ),
        ZoneState(
            zone_id="Z3", name="Industrial South",
            disaster_type=DisasterType.EARTHQUAKE,
            severity=ZoneSeverity.MEDIUM,
            population=1200, trapped_casualties=20, injured=60,
            rescued=0, fatalities=0, accessibility=0.9
        ),
    ],
...
def grade_task(task_id: str, env_state: Dict[str, Any]) -> float:
    ...
    attended_count = sum(
        1 for z in zones
        if z.zone_id in critical_zone_ids and z.rescued > 0
    )
