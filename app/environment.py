...
RESCUE_RATE: Dict[Any, int] = {
    DisasterType.EARTHQUAKE: 6,  DisasterType.FLOOD: 5,
    DisasterType.FIRE:       4,  DisasterType.MULTI: 5,
    "earthquake": 6, "flood": 5, "fire": 4, "multi": 5,
}
TREATMENT_RATE     = 12
FIREFIGHT_RATE     = 0.25
WATER_RESCUE_RATE  = 7
...
class DisasterResponseEnv:
    ...
    def reset(self) -> Observation:
        ...
        self._resources_total    = (
            self._resources.search_rescue_teams + self._resources.medical_teams
            + self._resources.firefighting_units + self._resources.water_rescue_teams
    ...
    def _apply_action(self, action: Action) -> Tuple[str, float]:
        ...
        if at == "allocate_search_rescue":
            ...
            waste    = 0.05 * over
            feedback = f"Deployed {deploy} S&R teams to {zone.name}." + (f" {over} over-requested." if over else "")
            ...
            self._resources_used += deploy
            waste    = 0.05 * over
            feedback = f"Deployed {deploy} medical teams to {zone.name}." + (f" {over} over-requested." if over else "")
            ...
            if zone.trapped_casualties > 10:
                return f"Cannot evacuate {zone.name} — {zone.trapped_casualties} still trapped.", 0.0
    ...
    def _simulate_dynamics(self) -> None:
        for zone in self._zones:
            ...
            # ISSUE 7 fix — explicit lookup with fallback
            ...
            if zone.firefighting_units > 0 and zone.disaster_type in ("fire", "multi"):
                zone.accessibility = min(1.0, zone.accessibility + zone.firefighting_units * FIREFIGHT_RATE)
            ...
            if not has_teams and zone.trapped_casualties > 0:
                ...
                if zone.disaster_type in ("fire", "multi") and zone.turns_unattended >= FIRE_SPREAD_THRESH:
            ...
            zone.search_rescue_teams = zone.medical_teams = zone.firefighting_units = zone.water_rescue_teams = 0
    ...
    def _compute_reward(self, waste_penalty: float) -> Reward:
        zones       = self._zones
        lives_saved = sum(
            z.rescued * SEVERITY_INDEX.get(z.severity, 1) / 4.0 * 0.005
            for z in zones
        )
        unattended  = sum(z.turns_unattended * 0.02 for z in zones if z.trapped_casualties > 0)
        time_bonus  = 0.01 * (self._task_def.max_steps - self._time_step) / self._task_def.max_steps
        efficiency  = 0.02 if self._resources_used > 0 else -0.02
        evac_bonus  = sum(0.03 for z in zones if z.is_evacuated)

        raw   = lives_saved + efficiency + time_bonus + evac_bonus - waste_penalty - unattended
    ...
    def _build_info(self) -> EpisodeInfo:
        ...
        grade   = (
            "A" if score >= 0.85 else "B" if score >= 0.70 else
